...
def doublet_call(args):
    ...
    if args.method == "threshold":
        ...
        calls[top_idx] = 1
        threshold_used = float(scores[top_idx[-1]]) if n_d > 0 else None
    else:  # valley — histogram valley detection on the observed score distribution
...
def main():
    ...
    parser.add_argument("--method",            required=True, choices=["threshold", "top_n", "top_pct", "valley"])
