...
class H5ADHandler:
    ...
    def _table_columns(f, table_path: str):
        ...
        AnnData dataframes store the ordered column list in the ``column-order``
        attribute (see AnnData on-disk format). Nested columns appear as paths
        such as ``Region/Tooth #``. Older writers may use ``_column_names`` instead.
        Falling back to ``list(node.keys())`` is last-resort only: it incorrectly
        treats intermediate Groups (parents of nested columns) as columns.
        """
        ...
        node = f[table_path]
        if isinstance(node, h5py.Group):
            cols = node.attrs.get('column-order', None)
            if cols is None:
                cols = node.attrs.get('_column_names', None)
            if cols is None:
                cols = list(node.keys())
    ...
    @staticmethod
    def _decode_nullable_group(item: h5py.Group) -> np.ndarray | None:
        """Decode AnnData nullable-{integer,boolean,string-array} groups (values + mask)."""
        if not isinstance(item, h5py.Group) or "values" not in item or "mask" not in item:
            return None
        values = item["values"][:]
        ...
        if enc == "nullable-boolean" or values.dtype.kind == "b":
            return np.array(
                ["nan" if m else str(bool(v)) for v, m in zip(values.flat, mask.flat)],
                dtype=object,
            ).reshape(values.shape)
        if enc == "nullable-string-array" or values.dtype.kind in ("S", "U", "O"):
            out = []
            ...
            return np.array(out, dtype=object).reshape(values.shape)
        # nullable-integer (and any other numeric nullable): keep numeric, NaN for masked
    ...
    def _transfer_metadata(f, group_path, loom, result, n_cells, n_genes, orientation, existing_paths, silent_skip=False):
        ...
        for col in cols:
            ...
            # column-order may use nested paths (e.g. "Region/Tooth #"); flatten "/" for loom.
            loom_col = col.replace("/", "_")
            loom_path = f"/{'col' if orientation == 'CELL' else 'row'}_attrs/{loom_col}"
            ...
            if isinstance(node, h5py.Group):
                # h5py "in" only checks immediate children; nested column-order
                # paths like "Region/Tooth #" must be resolved via __getitem__.
                try:
                    item = node[col]
                except KeyError:
                    result.setdefault("warnings", []).append(
                        f"Skipping {group_path}/{col}: listed in column-order but missing from group."
                    )
                    continue
                ...
                # Nullable integer/boolean/string: {values, mask}
                elif isinstance(item, h5py.Group) and 'values' in item and 'mask' in item:
                    values = H5ADHandler._decode_nullable_group(item)
                    if values is None:
                        result.setdefault("warnings", []).append(
                            f"Skipping {group_path}/{col}: could not decode nullable group."
                        )
                        continue

                # Unknown / nested intermediate Group (e.g. parent of nested columns)
                elif isinstance(item, h5py.Group):
                    enc = item.attrs.get("encoding-type", None)
                    if isinstance(enc, bytes):
                        enc = enc.decode()
                    result.setdefault("warnings", []).append(
                        f"Skipping {group_path}/{col}: unsupported Group column "
                ...
                # Old-style categorical: codes in dataset, categories elsewhere
                    ...
                    result.setdefault("warnings", []).append(
                        f"Skipping {group_path}/{col}: unsupported HDF5 type {type(item).__name__}."
                    )
                    continue
...
class DBManager:
    ...
    def get_genes_in_db(self, organism_id: int) -> MapGene:
        ...
        for r in rows:
            ...
            # By ensembl_id
            if g.ensembl_id:
                for key in MapGene._identifier_variants(g.ensembl_id):
                    self.map_gene._add_index_hit(self.map_gene.ensembl_db, key, g)

            # By gene name (exact symbol/name lookup; do not add numeric/prefixed variants here)
            if g.name:
                key = str(g.name).strip().upper()
                self.map_gene._add_index_hit(self.map_gene.gene_db, key, g)

            # alt_names can contain symbols, RefSeq accessions, NCBI Gene/Entrez IDs, etc.
            alt_names_raw = r.get("alt_names")
            for token in MapGene._iter_alias_tokens(alt_names_raw):
                token_up = token.upper()
                g.alt_names.add(token_up)
                for key in MapGene._identifier_variants(token):
                    self.map_gene._add_index_hit(self.map_gene.alt_db, key, g)

            # obsolete_alt_names are equally useful for old matrices/publication-era IDs.
            obsolete_raw = r.get("obsolete_alt_names")
            for token in MapGene._iter_alias_tokens(obsolete_raw):
                token_up = token.upper()
                g.obsolete_alt_names.add(token_up)
                for key in MapGene._identifier_variants(token):
                    self.map_gene._add_index_hit(self.map_gene.obsolete_db, key, g)
...
class Gene:
    ...
    # How this input identifier matched the DB: ensembl_id, name, alt_name,
    # obsolete_alt_name, or None for DB source records. Useful for debugging.
    match_source: Optional[str] = None
    # All release intervals that were compatible with the input identifier.
    # For ambiguous symbols/aliases this preserves alternatives for release
    # guessing, instead of only using the latest DB hit selected for metadata.
    release_intervals: List[Tuple[int, int]] = field(default_factory=list)
...
class MapGene:
    ...
    @staticmethod
    def _iter_alias_tokens(raw) -> list[str]:
        """Return alias tokens from DB values stored as text, text[], JSON, or CSV."""
        if raw is None:
            return []
        if isinstance(raw, (list, tuple, set, np.ndarray)):
            out = []
            for x in raw:
                out.extend(MapGene._iter_alias_tokens(x))
            return out
        ...
        # JSON arrays are common when aliases are exported from ETL steps.
        if s.startswith("[") and s.endswith("]"):
            try:
                loaded = json.loads(s)
                if isinstance(loaded, (list, tuple)):
                    return MapGene._iter_alias_tokens(loaded)
            except Exception:
                pass

        # PostgreSQL text[] values can arrive as '{a,b,c}' if cast to text.
        if s.startswith("{") and s.endswith("}"):
            s = s[1:-1]
    ...
    def _identifier_variants(value: str) -> list[str]:
        """Generate robust lookup keys for Ensembl/RefSeq/NCBI/Entrez aliases."""
        if value is None:
            return []
        raw = str(value).strip()
        if not raw:
        ...
        def add(x):
            ...
            if not x:
                return
            x_up = x.upper()
            compact = re.sub(r"\s+", "", x_up)
            for y in (x_up, compact, MapGene._VERSION_PATTERN.sub("", x_up), MapGene._VERSION_PATTERN.sub("", compact)):
                if y and y not in variants:
                    variants.append(y)

        add(raw)
        compact = re.sub(r"\s+", "", raw.upper())

        # Accept both prefixed and bare numeric forms, e.g. NCBIGene:12345,
        # GeneID:12345, EntrezGene:12345, and 12345.
        tails = []
        if ":" in compact:
            tails.append(compact.rsplit(":", 1)[1])
        if compact.startswith("NCBIGENE") and compact[len("NCBIGENE"):].isdigit():
            tails.append(compact[len("NCBIGENE"):])
        if compact.startswith("GENEID") and compact[len("GENEID"):].isdigit():
            tails.append(compact[len("GENEID"):])
        if compact.startswith("ENTREZGENE") and compact[len("ENTREZGENE"):].isdigit():
            tails.append(compact[len("ENTREZGENE"):])
        if compact.isdigit():
            tails.append(compact)

        for tail in tails:
            tail = MapGene._VERSION_PATTERN.sub("", tail)
            add(tail)
            if tail.isdigit():
                for prefix in ("NCBIGENE", "NCBIGENE:", "GENEID", "GENEID:", "ENTREZGENE", "ENTREZGENE:", "ENTREZ", "ENTREZ:"):
                    add(prefix + tail)

        return variants

    @staticmethod
    def _add_index_hit(index: Dict[str, List[Gene]], key: str, gene: Gene) -> None:
        if not key:
            return
        bucket = index.setdefault(key, [])
        if all(existing is not gene for existing in bucket):
            bucket.append(gene)

    @staticmethod
    def _gene_interval(g: "Gene") -> tuple[int, int] | None:
    ...
    @staticmethod
    def _release_intervals_from_hits(db_hits: List["Gene"]) -> list[tuple[int, int]]:
        seen = set()
        intervals = []
        for hit in db_hits or []:
            iv = MapGene._gene_interval(hit)
    ...
    @staticmethod
    def _lookup_index(index: Dict[str, List[Gene]], variants: list[str]) -> list[Gene] | None:
        for key in variants:
            hits = index.get(key)
            if hits:
                return hits
    ...
    def guess_ensembl_release(parsed_genes: List["Gene"], assemblies: list = None) -> dict:
        """
        Estimate the Ensembl release from gene release intervals.

        Scoring rule:
          1. maximize n_noobs_hits: direct ensembl_id/name matches only
          2. if tied, maximize n_hits: all matches, including alt/obsolete aliases
          3. if still tied, prefer the oldest assembly
          4. if still tied, prefer the earliest release within that assembly

        Returns:
          {
            "ensembl_release": int | None,
            "ensembl_assembly": str | None,
            "n_genes_supporting": int,              # direct-match support for chosen release
            "n_genes_considered": int,
            "n_genes_supporting_all_matches": int,  # all-source support for chosen release
          }
        """
        assemblies = assemblies or []

        gene_options: list[list[tuple[int, int]]] = []
        # Same intervals, but only for identifiers that matched directly by
        # Ensembl ID or current gene name. This intentionally excludes
        # alt_names and obsolete_alt_names so debug output can show how much
        # release support comes from direct/non-alias matches.
        noobs_gene_options: list[list[tuple[int, int]]] = []
        direct_match_sources = {"ensembl_id", "name"}
        for g in parsed_genes:
            ...
            if not opts:
                iv = MapGene._gene_interval(g)
                if iv:
                    opts = [iv]

            # Deduplicate and normalize intervals per input gene. One input gene
            # should contribute at most one hit to any release, even if several
            # aliases point to records present in the same release.
            normalized = []
            seen = set()
            for lo, hi in opts:
                if hi <= 0:
            ...
            if not normalized:
                continue
            gene_options.append(normalized)
            match_source = getattr(g, "match_source", None) or "unknown"
            if match_source in direct_match_sources:
                noobs_gene_options.append(normalized)

        n_considered = len(gene_options)
        if n_considered == 0:
            return {"ensembl_release": None, "ensembl_assembly": None,
                    "n_genes_supporting": 0, "n_genes_considered": 0,
                    "n_genes_supporting_all_matches": 0}

        min_release = min(lo for opts in gene_options for lo, _ in opts)
        max_release = max(hi for opts in gene_options for _, hi in opts)

        def coverage(r: int, options: list[list[tuple[int, int]]]) -> int:
            return sum(1 for opts in options if any(lo <= r <= hi for lo, hi in opts))

        def assembly_for_release(r: int) -> str | None:
            containing = [a for a in assemblies
                          if a.get("first_ensembl_release") and a.get("latest_ensembl_release")
                          and a["first_ensembl_release"] <= r <= a["latest_ensembl_release"]]
            if containing:
                return sorted(containing, key=lambda a: a["first_ensembl_release"])[0].get("name")
            return None

        # Lower rank means older assembly. Unknown assemblies are sorted after known assemblies.
        assembly_rank_by_name = {}
        for a in sorted(assemblies, key=lambda x: (x.get("first_ensembl_release") or 10**9,
                                                   x.get("latest_ensembl_release") or 10**9,
                                                   x.get("name") or "")):
            name = a.get("name")
            if name is not None and name not in assembly_rank_by_name:
                assembly_rank_by_name[name] = len(assembly_rank_by_name)

        release_counts = []
        for r in range(int(min_release), int(max_release) + 1):
            assembly = assembly_for_release(r)
            release_counts.append({
                "release": int(r),
                "n_hits": int(coverage(r, gene_options)),
                "n_noobs_hits": int(coverage(r, noobs_gene_options)),
                "assembly": assembly,
                "assembly_rank": assembly_rank_by_name.get(assembly, 10**9),
            })

        # Primary score is direct/non-alias coverage. All-source coverage is only
        # a tie-breaker, followed by oldest assembly and then earliest release.
        best = max(
            release_counts,
            key=lambda x: (x["n_noobs_hits"], x["n_hits"], -x["assembly_rank"], -x["release"])
        )
        best_release = int(best["release"])
        best_direct_cov = int(best["n_noobs_hits"])
        best_all_cov = int(best["n_hits"])

        # Map the chosen release to an assembly whose range contains it. If the
        # DB has assembly ranges with gaps, keep the previous fallback behavior.
        assembly_name = best.get("assembly")
        if assemblies and best_release is not None and assembly_name is None:
            below = [a for a in assemblies if a.get("latest_ensembl_release") and a["latest_ensembl_release"] <= best_release]
            if below:
                assembly_name = sorted(below, key=lambda a: a["latest_ensembl_release"], reverse=True)[0].get("name")
            elif assemblies:
                newest = sorted(assemblies, key=lambda a: (a.get("latest_ensembl_release") or 0), reverse=True)[0]
                assembly_name = newest.get("name")

        return {"ensembl_release": best_release,
                "ensembl_assembly": assembly_name,
                "n_genes_supporting": best_direct_cov,
                "n_genes_considered": int(n_considered),
                "n_genes_supporting_all_matches": best_all_cov}
    ...
    def parse_gene(self, query: str, index: int) -> Gene:
        ...
        if query:
            q_up = str(query).strip().upper()
            q_nover = self._VERSION_PATTERN.sub("", q_up)
            identifier_variants = self._identifier_variants(q_up)

            lookup_plan = [
                # Ensembl IDs first, including version-stripped forms.
                ("ensembl_id", self.ensembl_db, [q_up, q_nover] if q_nover != q_up else [q_up]),
                # Gene symbols/names keep exact symbol semantics.
                ("name", self.gene_db, [q_up]),
                # Alternative/obsolete names get robust NCBI/RefSeq/Entrez variants.
                ("alt_name", self.alt_db, identifier_variants),
                ("obsolete_alt_name", self.obsolete_db, identifier_variants),
            ]

            for source, index_db, variants in lookup_plan:
                db_hit = self._lookup_index(index_db, variants)
                if db_hit:
                    match_source = source
                    break
        ...
        # If found in DB
        if db_hit:
            g_src = self.retrieve_latest(db_hit)
            if g_src is None:
                db_hit = None
            else:
                # Do not mutate the shared DB Gene instance: copy it and attach
                # input-specific debugging/release-guess metadata.
                g_hit = dataclasses.replace(
                    g_src,
                    name=(g_src.name or g_src.ensembl_id),
                    alt_names=set(g_src.alt_names),
                    obsolete_alt_names=set(g_src.obsolete_alt_names),
                    match_source=match_source,
                    release_intervals=self._release_intervals_from_hits(db_hit),
                )
                return g_hit

        # If NOT found (Create fallback)
        fallback_name = query if query else f"Gene_{index + 1}"
        return Gene(ensembl_id="__unknown", name=fallback_name, biotype="__unknown", chr="__unknown", sum_exon_length=0)
