#!/usr/bin/env Rscript
# tsne.R — t-SNE on scRNA-seq data from a LOOM file using Seurat v5.
#
# Reads PCA cell embeddings from a LOOM file, runs t-SNE, appends the t-SNE
# cell embeddings into the input LOOM, and writes output.json.
#
...
HELP_TEXT <- "
t-SNE Script — Seurat v5 backend

Reads PCA cell embeddings from a LOOM file, runs Seurat v5 RunTSNE, appends
t-SNE embeddings directly into the input LOOM file, and writes output.json.
...
Options:
  ...
  --output_meta      Path where t-SNE embeddings (cells x n_components) are [required]
                     stored in the LOOM file.
                       Example: /col_attrs/tsne_R_scaled_ln
  --method           t-SNE method: RunTSNE                                  [required]
  -o / --output_dir  Output folder for output.json.
                     [optional, default: stdout]
  --assay            Seurat assay name.              [default: RNA]

  -- RunTSNE parameters (defaults match Seurat v5 RunTSNE) ────────────────────────
...
Output JSON tsne block:
  tool          str  — always 'Seurat'
  tool_version  str  — installed Seurat version
  n_components  int  — number of t-SNE dimensions
  n_dims        int  — number of PCA dimensions used
  perplexity    num  — perplexity parameter

Output JSON metadata entry:
  name          str  — LOOM-internal path of t-SNE embeddings (= --output_meta)
  on            str  — always 'CELL'
  type          str  — always 'NUMERIC'
  nber_rows     int  — number of cells
  nber_cols     int  — number of t-SNE dimensions
...
option_list <- list(
  ...
  make_option("--output_meta",         type = "character", default = NULL,  help = "Path for t-SNE embeddings (e.g. /col_attrs/tsne_R_scaled_ln). [required]"),
  make_option("--method",              type = "character", default = NULL,  help = "t-SNE method: RunTSNE. [required]"),
  make_option("--assay",               type = "character", default = "RNA", help = "Seurat assay name. [default: RNA]"),
  make_option("--n_dims",              type = "integer",   default = NULL,  help = "[RunTSNE] PCA dims to use (NULL = all). [default: NULL]"),
  make_option("--n_components",        type = "integer",   default = 2L,    help = "[RunTSNE] t-SNE dimensions. [default: 2]"),
  make_option("--perplexity",          type = "double",    default = 30.0,  help = "[RunTSNE] Perplexity. [default: 30]"),
  make_option("--seed_use",            type = "integer",   default = 42L,   help = "[RunTSNE] Random seed. [default: 42]")
)
...
valid_methods <- c("RunTSNE")
if (!args$method %in% valid_methods) ErrorJSON(paste0("Unknown method '", args$method, "'. Valid: ", paste(valid_methods, collapse = ", "), "."))
if (!startsWith(args$output_meta, "/")) ErrorJSON(paste0("--output_meta must start with / (e.g. /col_attrs/tsne), got: '", args$output_meta, "'."))
...
result <- list(
  parameters = list(),
  tsne       = list(tool = "Seurat", tool_version = seurat_version,
...
if (args$perplexity >= max_perplexity) {
  warn_env$w   <- c(warn_env$w, paste0("perplexity (", args$perplexity, ") capped to ", max_perplexity, " (must be < n_cells/3)."))
  args$perplexity <- as.double(max_perplexity)
}
...
result$tsne$perplexity <- args$perplexity

## ── Create Seurat object with PCA reduction and run t-SNE ────────────────────

fake_counts <- Matrix::Matrix(0, nrow = 1L, ncol = n_cells, sparse = TRUE,
                               dimnames = list("dummy_gene", cell_ids))
seurat_obj  <- suppressWarnings(CreateSeuratObject(counts = fake_counts, assay = args$assay, project = "tsne_r"))
...
seurat_obj <- capture_warnings(suppressMessages(RunTSNE(
  seurat_obj,
  reduction    = "pca",
  dims         = seq_len(n_dims),
  dim.embed    = args$n_components,
  perplexity   = args$perplexity,
  seed.use     = args$seed_use,
  verbose      = FALSE
)))

tsne_embeddings <- Embeddings(seurat_obj, reduction = "tsne")   # (n_cells x n_components)
...
size_tsne <- write_2d_dataset(h5_loom, out_path_h5, tsne_embeddings, n_cells, args$n_components)
h5_loom$close_all()
