...
# Arguments
input_loom_list <- args[1]
input_dataset_list <- args[2]
output_loom_path <- args[4]
output_dataset_path <- args[4]
n_pcs <- as.numeric(args[6]) # Number of PCs of PCA to use for computing UMAP
resolution_param <- as.numeric(args[7]) # Number of PCs of PCA to use for computing UMAP

#input_loom <- "/data/gardeux/07cuwi_parsing_output.loom,/data/gardeux/hxta68_parsing_output.loom"
#raw_dataset_path <- "/matrix"
#pca_dataset_path <- "/col_attrs/PCA_seurat"
#output_dataset_path <- "/col_attrs/Clustering_seurat"
#n_pcs <- 50 # Default
...
# Run Clustering
data.seurat <- FindNeighbors(data.seurat, dims = 1:n_pcs, verbose = F)
data.seurat <- FindClusters(data.seurat, resolution = resolution_param, verbose = F)
